...
@Api(description = "the document API")
@Component
public class CitationApi {

    private static  final Logger log = LoggerFactory.getLogger(CitationApi.class);


    private final DocumentApiService controller;

    @Autowired
    public CitationApi(DocumentApiService controller) {
    ...
    public Response getDocumentsByCursor(@ApiParam(value = "cursor id", required = true) @PathParam("token") String token) throws NotFoundException {
        ...
        try {
            response = controller.getDocumentsByCursor(token, true, false, false);
    ...
    public Response getDocuments(
        ...
        try {
            if (ids != null) {
                ...
                response = controller.getDocuments(idList, true, false, false);
                ...
                try {
                    response = controller.getDocumentsBySearchQuery(query, startYear, startMonth, true, false, false);
                } catch (BusyException e) {
                    log.error("an error occur, importer is busy : " + e.getMessage());
                    response = ResponseService.BuildBusyResponse(e.getMessage());
                } catch (Exception e) {
